...
class Convert:
    ...
    def load_data_helper(self, file_label):
        try:
            self.df = self.file_manager.load_data(file_label, self.convert_screen, show_success=True)
            self.file_manager.insert_tv(self.tv_c_1, self.df)
            if(self.info_label == None):
                self.info_label = Label(self.convert_screen, text = "Conversion is success. Download as either Excel or CSV \nby simply specifying the extension.")
    ...
    def convert_screen(self,root, img):
        ...
        self.convert_screen.geometry("450x600")
        ...
        self.tv_c_1 = self.file_manager.create_treeview(self.convert_screen, 300, 400, 0.5, 0.275, anchor="center", frame_text="Data Frame 1")
